...
class AsyncTokenAuthentication(BaseAuthentication):
    ...
    This class is needed if you use knox for authentication.

    This is an edited version of knox/auth.py
    ----
    ...
    async def authenticate(self, request):
        ...
        user, auth_token = await self.authenticate_credentials(auth[1])
    ...
    async def authenticate_credentials(self, token):
        ...
        async for auth_token in AuthToken.objects.filter(
                token_key=token[:CONSTANTS.TOKEN_KEY_LENGTH]).select_related('user'):

            if await self._cleanup_token(auth_token):
                continue
            ...
            if compare_digest(digest, auth_token.digest):
                ...
                return await self.validate_user(auth_token)
        raise exceptions.AuthenticationFailed(msg)

    async def renew_token(self, auth_token):
        ...
        if delta > knox_settings.MIN_REFRESH_INTERVAL:
            await auth_token.save(update_fields=('expiry',))

    async def validate_user(self, auth_token):
    ...
    async def _cleanup_token(self, auth_token):
        async for other_token in AuthToken.objects.filter(user=auth_token.user_id).select_related('user'):
            if other_token.digest != auth_token.digest and other_token.expiry:
                if other_token.expiry < timezone.now():
                    await other_token.delete()
